...
from utils.state_manager   import (load_h1_state, save_h1_state,
...
def run_m5_mode():
    ...
    for symbol in SYMBOLS:
        try:
            ...
            if already_open:
                ...
                remaining, closed_trades, context = monitor_cycle(open_trades, symbol)
                all_open_trades.extend(remaining)
                # Move closed trades to closed_trades list in state
            ...
            remaining, closed_now, context = monitor_cycle(open_trades, symbol)

            all_open_trades.extend(remaining)

            # Move trades closed this cycle into closed_trades in state
            if closed_now:
    ...
    # Fetches from Deriv + merges with daily_log for full detail.
    # Clears the daily_log after sending so it's fresh tomorrow.
    now = datetime.utcnow()
        ...
        if last_report != today:
            try:
                ...
                print(f"[M5] Daily report sent for {today} "
                      f"({len(log_trades)} logged trades)")
            except Exception as e:
