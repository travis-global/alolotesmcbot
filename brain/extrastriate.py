...
# Forex and gold close Friday 21:00 UTC — leave open over
# weekend and news gaps blow through SL without execution.
...
def _should_weekend_close(symbol: str) -> bool:
    """
    Returns True if this symbol should be force-closed before
    the weekend market closure.

    Rule: Friday 20:30 UTC onwards — gives 30-minute buffer
    before the 21:00 UTC close so we don't get caught in
    the closing spread spike.

    Synthetics (Volatility, Crash, Boom, etc.) are excluded —
    they trade 24/7 and are not affected by weekend closures.
    """
    if symbol not in _WEEKEND_CLOSE_SYMBOLS:
        return False
    now = datetime.utcnow()
    return now.weekday() == 4 and now.hour >= 20  # Friday from 20:00 UTC
def monitor_cycle(active_trades, symbol):
    ...
    for trade in active_trades:
        ...
        # ── Weekend close — before any other check ─────────
        # Force-close forex/gold positions Friday 20:00+ UTC.
        # A weekend gap can jump past SL with no execution,
        # turning a small loss into a much larger one.
        ...
        if sl_tp_hit:
            # Use the actual SL/TP price level for PnL — NOT the Deriv
            # contract's sold_for value which is in USD (e.g. $1.18),
            # not in price units (e.g. 0.69957). Using sold_for as the
            # close price produces absurd PnL like -4000 pips.
            close_price = trade["sl"] if exit_via == "sl" else trade["tp"]
            close_mt5_position(trade)   # close the Deriv contract (return ignored)
        ...
        if not closed:
            ...
            if violated:
                close_mt5_position(trade)   # close the Deriv contract (return ignored)
                # Use the candle close as market price — not sold_for contract value
