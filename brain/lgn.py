...
# CONFIG
# =========================================================
ENTRY_BUFFER      = 0.00015   # 1.5 pips — minimum move in trade direction
                               # before signal is emitted
ZONE_PROXIMITY    = 0.00050   # 5 pips — how close price must be to a zone
                               # before LGN starts watching it
...
def _confirm_trendline(tl, m5_data):
    ...
    for i in range(len(m5_data) - 1, max(0, len(m5_data) - 10), -1):
        candle = m5_data[i]
