...
async def _fetch_daily_trades_async(date_str: str) -> list:
    """
    Fetches all closed contracts for a given date from Deriv.
    date_str format: "2026-06-21"
    Returns list of trade dicts with standardised fields.
    """
    import time as _time

    # Calculate UTC day boundaries as Unix timestamps
    day_start = datetime.strptime(date_str, "%Y-%m-%d")
    day_end   = datetime(day_start.year, day_start.month, day_start.day,
                         23, 59, 59)
...
def fetch_daily_trades(date_str: str) -> list:
    """Fetches today's closed contracts from Deriv. Returns [] on failure."""
    try:
