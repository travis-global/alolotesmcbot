...
async def _place_order_async(symbol: str, direction: str,
    ...
    # Deriv minimum per limit_order value is $0.10 — skip if below that.
    # Extrastriate monitors price levels and closes trades independently anyway.
    if entry and sl and tp:
        try:
            ...
            if sl_usd >= 0.10 and tp_usd >= 0.10:
                params["limit_order"] = {
                    "stop_loss":   sl_usd,
                    "take_profit": tp_usd,
                }
            else:
                print(f"[Deriv] {symbol}: limit_order too small "
                      f"(sl=${sl_usd} tp=${tp_usd}) — placing without SL/TP")
        except Exception:
            pass
