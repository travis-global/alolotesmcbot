...
async def _place_order_async(symbol: str, direction: str,
    ...
    multiplier    = 100   # 100x leverage on multiplier contract

    # Build parameters — basis/amount/multiplier are required for MULT contracts.
    # duration/duration_unit are for binary options only — omit them here.
    params = {
        "amount":        stake,
        "basis":         "stake",
    ...
    # Add SL/TP as limit orders if price levels provided
    if entry and sl and tp:
        try:
            position_value = stake * multiplier
            sl_dist        = abs(entry - sl) / entry
            tp_dist        = abs(tp - entry) / entry
            sl_usd         = round(position_value * sl_dist, 2)
            tp_usd         = round(position_value * tp_dist, 2)
            ...
            pass   # proceed without limit orders if calculation fails
