...
async def _place_order_async(symbol: str, direction: str,
                              stake: float) -> tuple:
    # Always get a fresh OTP URL — OTP tokens are single-use
    ...
    # Include Deriv-App-ID header — required by the OTP WebSocket endpoint
    async with websockets.connect(
        ws_url,
        extra_headers={"Deriv-App-ID": DERIV_APP_ID}
    ) as ws:
