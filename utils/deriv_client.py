...
async def _place_order_async(symbol: str, direction: str,
    ...
    multiplier    = 10              # 10x — available on both forex and synthetics
    actual_stake  = max(stake, 1.0) # Deriv minimum stake is $1

    # Required fields per official schema:
    # https://developers.deriv.com/schemas/buy_request.schema.json
    # Required: contract_type, currency, underlying_symbol (NOT "symbol")
    params = {
        "contract_type":    contract_type,
        "currency":         "USD",
        "underlying_symbol": deriv_sym,  # schema requires "underlying_symbol" not "symbol"
        "amount":           actual_stake,
        "basis":            "stake",
        "multiplier":       multiplier,
    }

    # limit_order values are plain numbers per schema — NOT objects.
    # stop_loss  = USD amount of loss at which contract closes (positive number)
    # take_profit = USD amount of profit at which contract closes (positive number)
    if entry and sl and tp:
        try:
            ...
            if sl_usd > 0 and tp_usd > 0:
                params["limit_order"] = {
                    "stop_loss":   sl_usd,   # plain number
                    "take_profit": tp_usd,   # plain number
                }
        except Exception:
            pass

    async with websockets.connect(ws_url) as ws:
        # "buy" must be string "1" when using parameters (per schema pattern)
        ...
        while True:
            ...
            if resp.get("error"):
                # Always print the actual Deriv error — never swallow it silently
...
def place_order(symbol: str, direction: str,
                ...
                tp: float     = None) -> tuple:
    try:
        return _run(_place_order_async(symbol, direction, stake, entry, sl, tp))
    except Exception as e:
