...
# Sent to your PERSONAL Telegram (not the broadcast channel)
# at 21:00 UTC every day. Shows every trade placed that day
# with full detail so you can review performance without
# logging into Deriv.
# =========================================================
# =========================================================
# 8. DAILY TRADE REPORT
# Fetches directly from Deriv — not from bot internal state.
# Internal state tracking is unreliable; Deriv always has
# the ground truth of what was placed, closed, won, lost.
# =========================================================
def notify_daily_report(date_str: str, open_trades: list = None):
    """
    Fetches today's closed contracts from Deriv and sends a
    detailed report to TELEGRAM_PERSONAL_ID.

    date_str   : "2026-06-21"
    open_trades: list of still-open trades from state (optional)
    """
    if not PERSONAL_ENABLED:
        print("[Telegram] TELEGRAM_PERSONAL_ID not set — daily report skipped")
        return

    # Fetch from Deriv directly
    try:
        from utils.deriv_client import fetch_daily_trades
        trades = fetch_daily_trades(date_str)
    except Exception as e:
        print(f"[Telegram] Daily report fetch error: {e}")
        trades = []
    ...
    result_emoji = "✅" if net_pnl > 0 else "❌" if net_pnl < 0 else "⚖️"
    ...
    lines = [
        ...
        f"Net P&L :  <b>${net_pnl:+.2f}</b>  {result_emoji}",
    ]
    ...
    if trades:
        ...
        for i, t in enumerate(trades, 1):
            ...
            lines += [
                ...
                f"{d_emoji} <b>{underlying}</b>  {t.get('direction','')}",
        ...
        lines.append("")
        lines.append("No closed trades on Deriv for this day.")

    # Still-open trades from state
    if open_trades:
        lines.append("")
    ...
    lines.append(f"\n⏱ Generated {_ts()}")
    _send("\n".join(lines), chat_id=TELEGRAM_PERSONAL_ID)
    """
    Sends a detailed daily trade report to TELEGRAM_PERSONAL_ID.

    closed_today : list of closed trade dicts for today
    open_trades  : list of trades still open at report time
    date_str     : "15 Jun 2026"
    """
    if not PERSONAL_ENABLED:
        print("[Telegram] TELEGRAM_PERSONAL_ID not set — daily report skipped")
        return
    ...
    wins   = [t for t in closed_today if (t.get("pnl_pips") or 0) > 0]
    losses = [t for t in closed_today if (t.get("pnl_pips") or 0) <= 0]
    net_pnl = sum(t.get("pnl_pips", 0) or 0 for t in closed_today)
    win_rate = round(len(wins) / total * 100) if total else 0
    ...
    lines = [
        ...
        f"",
        f"Trades closed  : <b>{total}</b>",
        f"✅ Wins        : {len(wins)} ({win_rate}%)",
        f"❌ Losses      : {len(losses)} ({100 - win_rate}%)",
        f"Net PnL        : <b>{net_pnl:+.1f} pips</b>  {result_emoji}",
    ]
    ...
    if closed_today:
        ...
        for i, t in enumerate(closed_today, 1):
            ...
            confluence = " | ".join(t.get("confluence", []) or [])
            placed     = (t.get("placed_time") or "")[:16]
            closed_at  = (t.get("close_time")  or "")[:16]
            duration   = ""
            if placed and closed_at:
                try:
                    from datetime import datetime as _dt
                    p = _dt.strptime(placed,   "%Y-%m-%d %H:%M")
                    c = _dt.strptime(closed_at,"%Y-%m-%d %H:%M")
                    mins = int((c - p).total_seconds() / 60)
            ...
            lines += [
                f"",
                f"<b>Trade {i} — {outcome}</b>",
                f"{d_emoji} <b>{t.get('symbol')}</b>  {t.get('pattern')}  "
                f"{direction.upper()}",
                f"Entry      : <code>{entry}</code>  at  {placed} UTC",
                f"SL / TP    : <code>{sl}</code>  /  <code>{tp}</code>",
                f"R:R        : {rr}",
                f"Reason     : {confluence or '—'}",
                f"Exit       : {exit_type} @ <code>{close_px}</code>",
                f"Outcome    : {pnl:+.1f} pips",
            ]
            if duration:
    ...
    # Send to personal chat, not the broadcast channel
    _send("\n".join(lines), chat_id=TELEGRAM_PERSONAL_ID)
